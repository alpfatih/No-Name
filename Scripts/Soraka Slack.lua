...
function calcHeal(ignited)
	local rank = player:GetSpellData(_W).level
...
-- MANA
function calcMana()
	local rank = player:GetSpellData(_E).level
...
function calcUlt(ignited)
	local rank = player:GetSpellData(_R).level
	if rank == 0 then return nil end
	if ignited == false then
...
function GetPlayers(team, includeDead, includeSelf)
	...
	for i=1, heroManager.iCount, 1 do
		...
		if member ~= nil and member.valid and member.type == "obj_AI_Hero" and member.team == team then
...
function AutoMana()
	...
	if player:GetSpellData(_E).level > 0 and player:CanUseSpell(_E) == READY then
...
function GetInfo()
	...
	if GetTickCount() - GoldRefreshTick > 9999 then
		if (player.gold - gold10SecAgo) > 0 then
			info[2][1] = true
	...
	if SetupDebug == true then PrintChat("" .. player.gold) end	
...
-- LOAD --
	...
	-- here checking timer
	BoL:addTickHandler(Timer,200)
	BoL:addMsgHandler(Toggle)
	
	if SetupAutoSilence or SetupAntiKarthus then 
		BoL:addCreateObjHandler(AutoSilence)
	end
	
	if SetupDrawInfo then 
		BoL:addDrawHandler(DrawInfo)
	end
	
	PrintChat("SORAKA SLACK >> SORAKA SLACKER v8 loaded!")
	...
	-- detect karthus
	if SetupAntiKarthus then
		players = GetPlayers(TEAM_ENEMY, false, false)
		for i=1,#players,1 do
			if players[i].charName == "Karthus" then karthus = players[i] PrintChat("SORAKA SLACK >> Karthus Detected!")  end
		end
		if karthus == nil then SetupAntiKarthus = false end
	end
