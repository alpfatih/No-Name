...
-- dsol = DrawingSpriteOverlay("path_from_scripts_folder",x,y,scale)    - scale is optional
function DrawingSpriteOverlay(path,x,y,scale)
    local dsol = {}
    dsol.path, dsol.orig_path = SCRIPT_PATH..path, SCRIPT_PATH..path
    dsol.position, dsol.orig_position = {x=x,y=y}, {x=x,y=y}
    dsol.scale, dsol_orig_scale = scale or 1, scale or 1
    return dsol
end
]]
